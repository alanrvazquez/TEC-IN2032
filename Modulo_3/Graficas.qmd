...
from shewhart_charts import *
```

En el cógido de arriba, indicamos que usaremos funciones que estan contenidas en el archivo "shewhart_charts.py".
...
## Construyendo gráficas de control
...
Para construir la gráfica $\bar{X}$ y $R$ usamos las función `xbar_r_chart()` del conjunto de funciones en `shewhart_charts`.

```{python}
#| echo: true
#| output: false

result = xbar_r_chart(data = pull_data, value_col = 'Fuerza', 
                      group_col = 'Subgrupo', figsize=(9, 7))
plt.show()
```

En la función, tenemos que

- `data`: es el objeto de Python que contiene los datos.
- `value_col`: Indica la columna que contiene las observaciones.
- `group_col`: Indica la columna que contiene los subgrupos.

El comando `plt.show()` imprime solo la gráfica.
...
result = xbar_r_chart(pull_data, value_col = 'Fuerza', 
                     group_col = 'Subgrupo', figsize=(9, 7))
plt.show()
```

## Mostrando los límites de control

</br>

Para mostrar los límites de control de la gráfica $\bar{X}$, usamos los siguientes comandos:
...
print("X-bar chart")
print("Center line:", summary["xbar_center"].iloc[0])
print("LCL:", summary["xbar_lcl"].iloc[0])
print("UCL:", summary["xbar_ucl"].iloc[0])
```
...
Y para mostrar los límites de control de la gráfica $R$, usamos los siguientes comandos:
...
print("\nR chart")
print("Center line:", summary["r_center"].iloc[0])
print("LCL:", summary["r_lcl"].iloc[0])
print("UCL:", summary["r_ucl"].iloc[0])
```
...
Recuerda que, para usar las gráficas y los promedios de los subgrupos deben de seguir una distribución normal. Para evaluar esto, primero calculamos los promedios por subgrupo usando la función `.groupby()` de **pandas**.
...
average_pull = (pull_data
                .groupby(['Subgrupo'])
                .agg('mean')
                )
```

La función `.groupby()` agrupa las observaciones de acuerdo a una variable.
...
## Prueba de Shapiro-Wilk in Python

</br>

Para probar normalidad en Python, usamos la función `shapiro()` de la librería **scipy.stats**.
...
ResultShapiro = shapiro(average_pull)

print(f"P-value: {ResultShapiro.pvalue:.4f}")
```

</br>

[*Como el valor p no es menor que 0.05, no rechazamos* $H_0$. Por tanto, los promedios siguen una distribución normal.]{style="color: darkred"}
...
El rango móvil promedio es $\bar{MR} = \frac{MR_2 + MR_3 + \cdots + MR_{k}}{k - 1}$.
...
Primero, carguemos los datos que están en el archivo "Viscosity.xlsx".

```{python}
#| echo: true
#| output: true

viscosity_data = pd.read_excel("Viscosity.xlsx")
viscosity_data.head()
```

## Recuerda

La variable `Muestra` es [**categorica**]{style="color: red"}! Entonces, debemos de informar a Python sobre esto usando la función `pd.Categorical()`.

```{python}
#| echo: true
#| output: true

viscosity_data['Muestra'] = pd.Categorical(viscosity_data['Muestra'])
```
...
Para construir la gráfica de control para valores indidivuales, usamos las función `imr_chart()` del conjunto de funciones en `shewhart_charts`.

```{python}
#| echo: true
#| output: false

result = imr_chart(data = viscosity_data, value_col = 'Viscosidad', 
                   figsize=(9, 7))
plt.show()
```
...
Para mostrar los límites de control de la gráfica individual para promedios, usamos los siguientes comandos:
...
print("Individuals chart")
print("Center line:", summary["i_center"].iloc[0])
print("LCL:", summary["i_lcl"].iloc[0])
print("UCL:", summary["i_ucl"].iloc[0])
```
...
Y para mostrar los límites de control de la gráfica individual para rangos, usamos los siguientes comandos:
...
print("\nMoving range chart")
print("Center line:", summary["mr_center"].iloc[0])
print("LCL:", summary["mr_lcl"].iloc[0])
print("UCL:", summary["mr_ucl"].iloc[0])
```
...
Los datos están en el archivo de excel “Jugos.xlsx”.

## En Python

Carguemos los datos que están en el archivo "Jugos.xlsx"

```{python}
#| echo: true
#| output: true

juice_data = pd.read_excel("Jugos.xlsx")
juice_data['Subgrupo'] = pd.Categorical(juice_data['Subgrupo'])
juice_data.head()
```

## 

Para construir la gráfica $p$, usamos las función `p_chart()` del conjunto de funciones en `shewhart_charts`.

```{python}
#| echo: true
#| output: false

result = p_chart(data = juice_data, defectives_col = 'LatasNoConformes', 
                 n_col = 'nLatas', figsize=(9, 7))
plt.show()
```
...
- `data`: es el objeto de Python que contiene los datos.
- `defectives_col`: Indica la columna que contiene el conteo de defectuosos.
- `n_col`: Indica la columna que contiene el tamaño de muestra.
...
Para mostrar los límites de control de la gráfica $p$, usamos los siguientes comandos:
...
print("p-chart")
print("Center line:", summary["center"].iloc[0])
print("LCL:", summary["lcl"].iloc[0])
print("UCL:", summary["ucl"].iloc[0])
```
...
Para construir la gráfica $np$, usamos las función `np_chart()` del conjunto de funciones en `shewhart_charts`.

```{python}
#| echo: true
#| output: false

result = np_chart(data = juice_data, defectives_col = 'LatasNoConformes', 
                 n_col = 'nLatas', figsize=(9, 7))
plt.show()
```

Los argumentos son los mismos que la gráfica $p$.
...
Para mostrar los límites de control de la gráfica $np$, usamos los siguientes comandos:
