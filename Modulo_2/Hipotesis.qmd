...
[La **hipótesis alternativa es la hipótesis que buscamos sustentar** con base en la información contenida en la muestra.]{style="color:#4a6c40"}
...
</br> Sea $T$ una variable aleatoria que sigue una distribución $t$ con $n-1$ grados de libertad.
:::
...
$$H_0: \mu_R = \mu_B \text{ contra } H_1: \mu_R \neq \mu_B$$
...
prueba_hip = ttest_ind(Res_CompR, Res_CompB, alternative = 'two-sided', 
                       equal_var = False)
```
...
Como el p-valor es menor que $\alpha=0.05$, [**rechazamos**]{style="color: #4682B4"} $H_0$.
...
prueba_hip = ttest_ind(Res_CompR, Res_CompB, alternative = 'less', 
                       equal_var = False)
```
...
prueba_hip = ttest_ind(Res_CompR, Res_CompB, alternative = 'greater', 
                       equal_var = False)
```
