...
Desafortunadamente, para calcular un IC en Python debemos de especificar explicitamente ciertos objetos, incluyendo el tamaño de muestra ($n$) y el promedio de la variable ($\bar{X}$). Esto se puede hacer con las funciones de **pandas** que ya vimos.

```{python}
#| echo: true

X = cilinder_data.filter(['Coeficiente'])
confianza = 0.95
n = cilinder_data.filter(['Coeficiente']).count()
media = cilinder_data.filter(['Coeficiente']).agg(["mean"])
```

. . .

Además, también necesitamos el *error estándar*, que es igual a $s/\sqrt{n}$. Para calcular esta cantidad, usamos la funcion `sem()` de **scipy.stats**.

```{python}
#| echo: true

desviacion_error = sem(X) 
```

## Intervalo de confianza en Python

</br></br>

Ahora, creamos el intervalo de confianza usando la función `t.interval()` de **scipy.stats**.

```{python}
#| echo: true

intervalo = t.interval(confianza, n-1, loc = media, 
                      scale = desviacion_error)
intervalo
```
...
Podemos visualizar los datos de los dos grupos definitos por los tubos del tipo 1 y 2 usando gráficas de cajas lado a lado.
...
El parámetro `equal_var` ene sta función indica si asumimos que las varianzas son iguales o diferentes. Asumamos que son diferentes.
