---
title: "Introducción a la Estadística"
subtitle: "IN2032: Análisis Estadístico de Datos"
author: 
  - name: Alan R. Vazquez
    affiliations:
      - name: Departmento de Ingeniería Industrial
format: 
  revealjs:
    chalkboard: false
    multiplex: false
    footer: "Tecnológico de Monterrey"
    logo: IN2032_logo.png
    css: style.css
    slide-number: True
    html-math-method: mathjax
editor: visual
jupyter: python3
---

## Agenda

</br>

- Introducción

- Muestreo

- Experimentos y estudios observacionales

- Resumenes estadísticos

# Introducción

## ¿Por qué la estadística?

</br>

::: incremental
- Sacar conclusiones confiables de los datos.

- Hacer frente a la incertidumbre en mediciones científicas repetidas.

- Diseñar experimentos válidos y sacar conclusiones fiables.

- Construir modelos o algoritmos para predecir eventos futuros.

- Ser un miembro bien informado de la sociedad.
:::

## Ejemplo 1

</br>

::::: columns
::: {.column width="60%"}
Una máquina fabrica varillas de acero para usar en dispositivos de almacenamiento óptico.

La especificación para el diámetro de las varillas es 0.45 ± 0.02 cm.

Durante la última hora, la máquina ha fabricado 1000 varillas.
:::

::: {.column width="40%"}
</br> ![](images/clipboard-3830659353.png){fig-align="center"}
:::
:::::

## 

::::: columns
::: {.column width="60%"}

El ingeniero de calidad quiere saber aproximadamente cuántas de estas varillas cumplen con la especificación.
:::

::: {.column width="40%"}
![](images/clipboard-3947900040.png){fig-align="center"}
:::
:::::

. . . 

::: {style="text-align: center;"}
[¡Pero no tiene tiempo para medir las 1000 varillas!]{style="color: darkred;"}
:::

## 

</br>

::::: columns
::: {.column width="60%"}
</br></br>

[**Solución**]{.underline}: tomar una muestra aleatoria de 50 varillas y medirlas.

Al hacer esto, el ingeniero descubre que 46 de ellas (92%) cumplen con la especificación de diámetro.
:::

::: {.column width="40%"}
![](images/clipboard-3717050976.png){fig-align="center"}
:::
:::::

## Sin embargo

</br></br></br>

- Es poco probable que la muestra de 50 barras represente perfectamente a la población de 1000.

- Es probable que la proporción de varillas buenas en la población difiera algo de la proporción de la muestra.

## 

</br></br>

El ingeniero debe responder preguntas basadas en los datos de muestra. Por ejemplo:

::: incremental
- ¿Qué intervalo da una buena estimación del porcentaje de varillas aceptables en la población con certeza razonable?

- ¿Qué tan seguro puede estar el ingeniero de que al menos el 90% de las varillas están en buenas condiciones?
:::

. . .

::: {style="text-align: center;"}
[**¡La estadística nos ayuda contestar preguntas como éstas!**]{style="color: #000080;"}
:::

# Muestreo

## Muestreo

Conceptos importantes preliminares:

::: incremental
1.  Una [**población**]{style="color: #000080;"} es la colección completa de objetos o resultados sobre los cuales se busca información.

2.  Una [**muestra**]{style="color: #000080;"} es un subconjunto de una población que contiene los objetos o resultados que realmente se observan.

3.  Una [**muestra aleatoria simple (MAS)**]{style="color: #000080;"} de tamaño $n$ es una muestra elegida mediante un método en el que cada colección de $n$ elementos de la población tiene la misma probabilidad de constituir la muestra, tal como en la lotería.
:::

## Ejemplo 2

En una universidad grande, una profesora está interesado en la altura promedio de los estudiantes de la universidad.

Obtiene una lista de los 50,000 estudiantes matriculados en la universidad y asigna un número a cada estudiante.

Ella utiliza un generador de números aleatorios por computadora para generar 100 números enteros aleatorios entre 1 y 50,000, y los estudiantes correspondientes a esos números son seleccionados para medir su altura.

</br>

. . .

::: {style="text-align: center;"}
[**Esta es una muestra aleatoria simple.**]{style="color: #2E6021;"}
:::

## Variación muestral

</br>

No se garantiza que una muestra aleatoria simple (MAS) refleje perfectamente la población.

Las MAS siempre difieren en algunos aspectos entre sí; ocasionalmente una muestra es sustancialmente diferente de la población.

Dos muestras diferentes de la misma población también variarán entre sí.

Este fenómeno se conoce como [**variación muestral**]{style="color: darkgreen;"}.

## De regreso al Ejemplo 1

En la muestra que recopiló el ingeniero, el 92% cumplía con las especificaciones.

</br>

De la población de 1000 barras, es poco probable que exactamente el 92% cumpla con las especificaciones.

</br>

Debido a la variación del muestreo, es más realista pensar que la verdadera proporción de varillas que cumplen con las especificaciones [será cercana a la proporción de la muestra, o 92%]{style="color: #B2AC88"}.

## Población conceptual

Una **población conceptual** consta de todos los valores que posiblemente podrían haberse observado en una población.

* Por ejemplo, un ingeniero mide varias veces el tiempo de ciclo de una estación de ensamblaje usando un cronómetro digital. Aunque el proceso sea el mismo, cada medición resulta ligeramente diferente debido a variaciones naturales del operador, el sistema y el instrumento de medición.

. . . 

Aquí la población es conceptual consta de todos los tiempos de ciclo que, en principio, podrían observarse para esa estación de trabajo.

## Independencia

Se dice que los elementos de una muestra son [**independientes**]{style="color: darkgreen"} si conocer los valores de algunos de ellos no ayuda a predecir los valores de los demás.

</br>

Los elementos de una muestra aleatoria simple pueden tratarse como independientes en la mayoría de los casos encontrados en la práctica.

</br>

Una excepción ocurre cuando la población es finita y la muestra comprende una fracción sustancial (más del 5%) de la población.

## 

Por ejemplo, un ingeniero industrial selecciona al azar piezas producidas durante distintos momentos del día para medir su diámetro. 

Saber que una pieza tiene un diámetro ligeramente mayor al objetivo [**no permite**]{style="color: darkgreen"} predecir el diámetro exacto de las demás piezas seleccionadas, ya que cada medición proviene de una producción diferente e independiente.

. . . 

En contraste, si se toman cinco piezas consecutivas producidas por la misma máquina justo después de un ajuste incorrecto, las mediciones probablemente serán similares entre sí. En este caso, [**conocer el valor de una pieza sí ayuda a predecir las demás, por lo que las observaciones no son independientes.**]{style="color: darkred"}

# Experimentos y Estudios Observacionales

## Estudios observacionales

Un [**estudio observacional**]{style="color: darkgreen"} es aquel en el que el científico simplemente observa un fenómeno y registra datos, sin tener ningún control sobre el fenómeno.

</br>

Los estudios observacionales son valiosos para descubrir tendencias y posibles asociaciones.

</br>

. . .

[Sin embargo, ¡no son tan buenos como los experimentos controlados para obtener conclusiones confiables sobre causa-y-efecto!]{style="color: red"}

## Experimentos controlados

Un experimento controlado es aquel en el que el científico tiene un grado de control sobre el fenómeno tal que le permite recolectar datos sobre el mismo de una manera estructurada.

Se pueden utilizar diferentes experimentos para generar datos:

:::: incremental
::: {style="font-size: 88%;"}
- Los **experimentos de una muestra** involucran solo una población de interés y de ella se extrae una única muestra.

- Los **experimentos de muestras múltiples** involucran dos o más poblaciones de interés y se extrae una sola muestra de cada población.
:::
::::

. . .

Los experimentos controlados permiten establecer relaciones de causa-y-efecto.

## Ejemplo 

[**Estudio observacional**]{style="color: darkgreen"}: Un ingeniero industrial analiza datos históricos de distintas plantas manufactureras y observa que las líneas de producción con mayor capacitación de operadores suelen tener menos productos defectuosos.

. . . 

[**Estudio experimental**]{style="color: darkblue"}: un ingeniero industrial diseña un experimento donde asigna aleatoriamente a dos grupos de operadores: un grupo recibe un nuevo programa de capacitación y el otro continúa con el método actual. Después, compara la tasa de defectos entre ambos grupos para evaluar si la capacitación realmente causa una mejora en la calidad.


# Resumenes estadísticos

## Tipos de datos I

</br></br>

Cuando se asigna una cantidad numérica que designa cuánto o cuántos a cada elemento de la muestra, el conjunto de valores resultante es [**numérico**]{style="color: #D8A7B1"} o [**cuantitativo**]{style="color: #D8A7B1"}.

- Altura (en centímetros).

- Peso (en kilogramos).

- Edad (en años).

## Tipos de datos II

</br></br>

Cuando los elementos de muestra se colocan en categorías y se asignan nombres de categorías a los elementos de muestra, los datos son [**categóricos**]{style="color: pink"} o [**cualitativos**]{style="color: pink"}.

- Color de cabello.

- País de origen.

- Código postal.

## Tipos de datos III

![](images/Images.002.jpeg){fig-align="center"}

## Resúmenes estadísticos

Una muestra suele ser una larga lista de números. Para ayudar a resaltar las características importantes de una muestra, **calculamos estadísticas resumidas**.

</br>

. . .

Para datos numéricos, las estadísticas resumidas más populares son:

::: incremental
- Muestra promedio.

- Variación de la muestra.

- Desviación estándar muestral.
:::

## Media muestral

</br></br>

Denotemos $y_1, y_2, \ldots, y_n$ observaciones como $n$. La [**media muestral (promedio)**]{style="color: #4682B4;"} es:

$$\bar{y} = \frac{1}{n}\sum_{i=1}^{n} y_i = \frac{y_1+y_2+\cdots+y_n}{n}.$$

La media muestral indica el centro de los datos.

## Varianza muestral

</br></br>

Denotemos $y_1, y_2, \ldots, y_n$ observaciones como $n$. La [**varianza muestral**]{style="color: #4682B4;"} es:

::: {style="font-size: 78%;"}
$$s^2 = \frac{1}{n-1} \sum_{i=1}^{n} (y_i - \bar{y})^2 = \frac{1}{n-1} \left( (y_1 - \bar{y})^2 + (y_2 - \bar{y})^2 +\cdots + (y_n - \bar{y})^2\right).$$
:::

La varianza muestral es como un promedio de las diferencias al cuadrado entre cada observación y la media muestral. Da una indicación de cuán dispersos están los datos.

## Desviación estándar muestral

::: {style="font-size: 90%;"}
[Un inconveniente de la varianza muestral es que no está en la misma escala que las observaciones reales.]{style="color: darkred;"} Esto se debe a que las diferencias entre cada observación y la media muestral se elevan al cuadrado.

Para obtener una medida de dispersión cuyas unidades sean las mismas que las de la muestra, simplemente tomamos la raíz cuadrada de la varianza muestral.

$$s =\sqrt{\frac{1}{n-1} \sum_{i=1}^{n} (y_i - \bar{y})^2}.$$

Esta cantidad se conoce como [**desviación estándar muestral**]{style="color: #4682B4;"}.
:::

## Estadísticas muestrales y parámetros de población

</br>

Cada estadística de muestra aquí tiene una contraparte de parámetro poblacional.

Los resúmenes numéricos de una **muestra** se llaman [**estadísticas**]{style="color: darkblue;"}.

Los resúmenes numéricos o características de una **población** se denominan [**parámetros**]{style="color: darkgreen;"}.

## 

</br></br>

::::: columns
::: {.column width="50%"}
Para la población:

- La media es $\mu$.

- La varianza es $\sigma^2$.

- La desviación estándar es $\sigma$.
:::

::: {.column width="50%"}
Para la muestra:

- La media (promedio) es $\bar{y}$.

- La varianza es $s^2$.

- La desviación estándar es $s$.
:::
:::::

</br>

. . .

[Uno de los objetivos de esta clase es aprender o estimar los parámetros de la población utilizando estadísticas de muestra.]{style="color: purple"}

## Resumen estadistico de variables categoricas

Los dos resúmenes numéricos más utilizados para datos categóricos son las frecuencias y las [**proporciones muestrales**]{style="color: darkgreen"} (a veces llamadas *frecuencias relativas*).

- La frecuencia de una categoría determinada es simplemente la cantidad de elementos de muestra que caen en esa categoría.

- La proporción muestral es la frecuencia dividida por el tamaño de la muestra.

## Preguntas de práctica para examen

</br></br>

- Explica la variación muestral.

- Define el concepto de independencia.

- ¿Qué es la desviación estándar? ¿Cómo se calcula?

# [Return to main page](https://alanrvazquez.github.io/TEC-IN2032/)
